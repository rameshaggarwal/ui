...
export default Route.extend({
  model() {
    return callApiNaive(`https://deltadevops.app:8553/engine-rest/task?sortBy=created&sortOrder=desc`).then((responses) => {
      const promises = [];
      const response = [];
      ...
      responses.forEach((responser) => {
        if (!responser.formKey) {
          ...
          promises.push(callApiNaive(`https://deltadevops.app:8553/engine-rest/task/${ responser.id }/form-variables`))
        }
      })
