  ...
  didInsertElement() {
    ...
    scheduleOnce('afterRender',this,() => {
      ...
      Formio.createForm(document.getElementById('formio'),`https://mkmtcojqfzqvaje.form.io/${this.get('formtype')}`).then(async (form) => {
        /*
        get(this,'globalStore').find('user',null,{
          forceReload: true,
          filter: {me: true}
        })
          .then((user) => {const x=EmberObject.create({account: get(user,'firstObject')}); console.log(Object.assign({},x));});
          */
        ...
        form.on('submitDone',(submitted) => {
          ...
          if(this.get('submissionId')!==0) {
            callApiNaive(`https://mkmtcojqfzqvaje.form.io/deltarequest/submission/${this.get('submissionId')}`).then((submissionValues) => {
              ...
              if(submitted.data.addComment) submissionValues.data.comments+=`${new Date().toUTCString()} -> <i>admin</i>: ${submitted.data.addComment}<br/>`
              if(submitted.data.email) submissionValues.data.comments+=`${new Date().toUTCString()} => <i>Task Assigned to</i> ${submitted.data.email}<br/>`
            ...
            callApiNaive(`https://mkmtcojqfzqvaje.form.io/deltarequest/submission`).then((response) => {
              response.forEach((response) => {
                if(response.data.id===this.get('submission.data.id')) {
                  ...
                  if(submitted.data.addComment) response.data.comments+=`${new Date().toUTCString()} -> <i>admin</i>: ${submitted.data.addComment}<br/>`
...
ExportedComponent.reopenClass({
  positionalParams: ['submissionId','submission','formtype','startform','taskid']
});
